    ...
    */
    public function get()
    {
      ...
      $this->posts['postsArray'] = $this->query($this->args);
        ...
        $typeTerms = $this->get_hierachical_terms('type' , null, array('hide_empty' => false), array('hide_empty' => false));
        // die(var_dump($typeTerms['all']->children));
        // if (($value = $typeTerms['residential'] ) !== false) {
        //   array_unshift ($typeTerms, $value);
        //   unset($typeTerms['residential']);
        // }

        // if (($value = $typeTerms['commercial'] ) !== false) {
        //   array_unshift ($typeTerms, $value);
        //   unset($typeTerms['commercial']);
        // }

        // if (($value = $typeTerms ) !== false ) {
        //   $typeTerms = array_merge( $typeTerms, $children );
        //   // die(var_dump($children));
        //   array_unshift ($typeTerms, $value);
        // }

        // die(var_dump($typeTerms));
      ...
      if( !empty($this->posts['postsArray']) ) {
        ...
        array_walk($this->posts['postsArray'], function(&$post, $key) {
            ...
            if( method_exists ($this, 'postDataCallback') ) {
              $single->post = $this->postDataCallback($single->post);
            }
						$post = $single->post;
        });
      }
