...
class Single extends Base
{
    ...
    **/
    public function get() {
      if ($this->post->object_type !== 'page') {
        $this->additionalPostData($this->post);
      }
    ...
    */
    public function addRelatedPosts () {
      // Base args
      $this->relatedPostsArgs = array( 'query' => array(
          'posts_per_page' => 3,
      ...
      if ($this->post->post_type == 'post') {
           $this->relatedPostsArgs['query']['category__in'] = $this->post->taxonomyObjs['category']->terms;
           $this->relatedPostsArgs['query']['post__not_in'] = array( $this->post->ID );
      } else {
        $this->relatedPostsArgs['query']['tax_query'] = [];
        foreach ($this->post->taxonomyObjs as $key => $taxonomyObj) {
          $this->relatedPostsArgs['query']['tax_query'][$key] = array('field' => 'id');
          $this->relatedPostsArgs['query']['tax_query'][$key]['taxonomy'] = $taxonomyObj->name;
          $this->relatedPostsArgs['query']['tax_query'][$key]['terms'] = $taxonomyObj->terms;
        }
      }
      ...
      $this->addToObj($this->post, 'relatedPosts', function( $postObj ) {
        if( !empty($this->relatedPostsArgs) ) {
          ...
          $relatedPosts = new \Controllers\Archive($this->relatedPostsArgs);
          $posts = $relatedPosts->returnData('posts');
