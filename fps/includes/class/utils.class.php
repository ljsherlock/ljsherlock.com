...
require_once RW_LIB . '/vendor/autoload.php';

class Utils
{
    /**
     *  Used within ::isFirstPost & ::resetFirstPost
     *
     *  @type [ String => Int ]
     */
    public static $firstPosts = array();


    /**
     *  Detect if the post we are currently checking is the first one
     *
     *  @param  Int     $post_id    optional
     *  @return Bool
     */
    public static function isFirstPost($_post_id = false)
    {
        $_post_id = (int) $_post_id;
        $post_id = !empty($post_id) ? $_post_id : get_the_id();
        $post_type = get_post_type($post_id);

        if (isset(static::$firstPosts[ $post_type ])) {
            /**
             *  Check if the id of the current post matches the one
             *  which has been set has first post.
             *
             *  Usefull when calling this function multiple time
             *  on 1 iteration of the loop
             */
            return static::$firstPosts[ $post_type ] === $post_id;
        }

        static::$firstPosts[ $post_type ] = $post_id;
        return true;
    }

    /**
     *  Reset First Post detection for a given post type
     *
     *  @param  String  $post_type
     *  @return Bool
     */
    public static function resetFirstPost($post_type)
    {
        if (isset(static::$firstPosts[ $post_type ])) {
            unset(static::$firstPosts[ $post_type ]);
            return true;
        }
    ...
    /**
     *  Get srcset attribute for image based on WP image sizes
     *
     *  @param  Int     $attachement_id
     *  @param  Array   $sizes
     *  @return String
     */
    public static function getSrcsetForWPSizes($attachement_id, $sizes)
    {
        if (!is_array($sizes) || !is_int($attachement_id)) {
            return false;
        }
        ...
        foreach ($sizes as $size) {
            $img = wp_get_attachment_image_src($attachement_id, $size);
            ...
            $image_width = $img[1];
            $image_height = $img[2];
            $image_is_generated = $img[3];

            // Already included in srcset
            if (in_array($image_src, $img_done)) {
                continue;
            }

            // Prevent Full size image to be displayed when not intended to
            if ($size !== 'full' && !$image_is_generated) {
                continue;
            }

            $img_done[] = $image_src;
            $srcset[] = "{$image_src} {$image_width}w";
        }
    ...
    /**
     *  Inline content of a file
     *
     *  @param  String  $filePath
     *  @param  Bool    $echo       - Echo directly
     *  @param  Bool    $use_min    - Use minified version of file if available
     *  @return String | False
     */
    public static function inlineFile($filepath, $echo = true, $use_min = false)
    {
        if ($use_min) {
            $matches = null;

            // Check if it's an asset ending with .js or .css
            preg_match('/\.(css|js)$/', $filepath, $matches);

            if (!empty($matches)) {
                $extension = $matches[0];
                $minpath = str_replace("{$extension}", ".min{$extension}", $filepath);

                // if a min version is available we use it
    ...
    /**
     *  Callback function to sort array of version numbers
     *
     *  @param  {string} $a - version number. ex: '1.0.2'
     *  @param  {string} $b - same as above
     *  @return {int} - possible values: -1, 0, 1
     */
    public static function orderChapterNumber($a, $b)
    {
        if (empty($a) || empty($b)) {
            return 0;
        }
        ...
        $version_a = explode('.', $a);
        $version_b = explode('.', $b);
        $a_count = count($version_a);

        for ($i = 0; $i < $a_count; $i++) {
            if (!isset($version_b[ $i ])) {
                return 1;
            }

            $a_int = (int) $version_a[ $i ];
            $b_int = (int) $version_b[ $i ];
